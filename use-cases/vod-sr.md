...
This use case describes how to build an automated VOD super-resolution (VOD-SR) processing pipeline using the Bluewhale AMI.
By combining AWS Systems Manager, Lambda, Step Functions, and MediaConvert, the system automatically performs SR processing on uploaded video files and generates HLS outputs without any manual intervention.

## Architecture

<br/>
<img src="../images/vod-sr/vod-sr.png" width="90%" style="display: block; margin: auto;">
<br/>

## Prerequisites

To successfully follow this use case, you should have a basic understanding of the AWS concepts:
...
- In the **Application and OS Images** search bar, enter _Bluewhale_
  - Select “Real-time AI Video Upscaling and Enhancement - Bluewhale”
...
- For **Command document**, select AWS-RunShellScript
- In the Command Parameters field, enter the following execution script : [2-1. Execution script(`exec_bluewhale_sr.sh`)](#2-1-execution-script-exec_bluewhale_srsh)
- Execute Run Command
- Under **Targets and outputs → Instance ID**, review the command output

#### 2-1. Execution script (`exec_bluewhale_sr.sh`)
...
In this step, you will configure an automated pipeline that generates HLS output for SR-processed files. When a new original file is uploaded to a designated S3 bucket, the pipeline is triggered and executed through AWS Lambda → Step Functions → AWS Systems Manager → AWS MediaConvert.
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Action": ["s3:GetObject"],
      "Resource": ["arn:aws:s3:::{orig_file-bucket}/*"]
    },
    {
      "Effect": "Allow",
      "Action": [
...
export const handler = async event => {
  ...
  try {
    ...
    // ex) 'arn:aws:states:ap-northeast-2:{id}:stateMachine:sr_exec_with_ssm_run_cmd';
...
```bash
arn:aws:iam::{iam_id}:role/service-role/s3_upload_trigger-role-03uddgtr
```
...
```json
{
  ...
  "States": {
    ...
    "MediaConvert_CreateJob": {
      ...
      "Parameters": {
        "Role": "arn:aws:iam::{iam_id}:role/MediaConvert_Default_Role",
        "Settings": {
          "TimecodeConfig": {
            "Source": "ZEROBASED"
          },
...
```json
{
  ...
  "origBucket": "{origfile_bucket}",
  "origFile": "origfile_sample.mp4",
  "srBucket": "{sr_bucket}",
  "srFile": "origfile_sample_sr_2x.mp4"
}
```
